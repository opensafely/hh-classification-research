*************************************************************************
*Do file: 17 Sensitivity analysis where the cohort is people aged over 60 and above.do
*
...
foreach outcome in covidHospOrDeath {
	...
	* Open a log file
	capture log close
	log using "./logs/17_SensAnalysis1_Age60_HR_3CATSFULLMVtable_`outcome'_`dataset'", text replace
	
	*open table
	file open tablecontents using ./output/17_SensAnalysis1_Age60_HR_3CATSFULLMVtable_`outcome'_`dataset'.txt, t w replace
	...
	*write table title and column headers
	...
	forvalues e=1/5 {
		use ./output/SensAnalysis1_Age60_dataset_STSET_`outcome'_ageband_3`dataset'.dta, clear
		...
		*hhrisk exposure 3 categories
		cap noisily outputHRsforvar, variable(hhRiskCatExp_4cats) catLabel(hhRiskCat60PLUS_4cats) min(1) max(4) ethnicity(`e') outcome(`outcome')
		*age (3 splines)
...
/*original code (all variables, rates and MV regression)
local dataset `1' 
