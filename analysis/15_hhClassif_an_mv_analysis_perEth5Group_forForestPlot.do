...
foreach outcome in covidDeath covidHosp covidHospOrDeath nonCovidDeath {
	...
	* Open a log file
	capture log close
	log using "./logs/15_hhClassif_forForestPlot_`outcome'_`dataset'", text replace
	
	*open table
	file open tablecontents using ./output/15_hhClassif_forForestPlot_`outcome'_`dataset'.txt, t w replace
