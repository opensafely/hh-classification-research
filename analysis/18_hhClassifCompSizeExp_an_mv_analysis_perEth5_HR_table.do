...
foreach outcome in covidDeath covidHosp covidHospOrDeath nonCovidDeath {
	...
	* Open a log file
	capture log close
	log using "./logs/18_hhClassifCompSizeExp_an_mv_analysis_perEth5_HR_table_`outcome'_`dataset'", text replace
	
	*open table
	file open tablecontents using ./output/18_hhClassifCompSizeExp_an_mv_analysis_perEth5_HR_table_`outcome'_`dataset'.txt, t w replace
	...
	*write table title and column headers
	...
	forvalues e=1/5 {
		...
		*include version with four exposure categories
		...
		cap noisily outputHRsforvar, variable(HHRiskCatCOMPandSIZE) catLabel(HHRiskCatCOMPandSIZE) min(1) max(14) ethnicity(`e') outcome(`outcome')
