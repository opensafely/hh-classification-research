...
foreach outcome in covidHospOrDeath  {
	...
	* Open a log file
	capture log close
	log using "./logs/20a_SENS_TPP100_`outcome'_`dataset'", text replace
	
	*open dataset
	use ./output/hhClassif_analysis_dataset_STSET_`outcome'_ageband_3`dataset'.dta, clear
	
	*keep only those people who have 100% tpp coverage in their houses
	keep if tpp100Percent==1
	count
	
	*open table
	file open tablecontents using ./output/20a_SENS_TPP100_`outcome'_`dataset'.txt, t w replace
