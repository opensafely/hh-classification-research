				...
				*get total count of people by for each ethnicity
				...
				forvalues i=`min'/`max' {
					...
					*get person time and rate and counts
					...
					local percentWEvent=100*(`event'/`n_people')
					...
					*write each row
					if `i'==1 {
						*write the total
						...
						file write tablecontents  _tab ("`category'") _tab (`n_people') _tab (`event') _tab %3.1f (`percentWEvent') _tab "1" _tab "1" _tab "1"  _n
					}
					else {
					file write tablecontents  _tab ("`category'") _tab (`n_people')  _tab (`event') _tab %3.1f (`percentWEvent') _tab %4.2f (`hr_mvAdjWHHSize') _tab  %4.2f (`lb_mvAdjWHHSize') _tab %4.2f (`ub_mvAdjWHHSize') _n
					}
...
foreach outcome in covidHospOrDeath {
	...
	* Open a log file
	capture log close
	log using "./logs/16_hhClassif_forForestPlot_`outcome'_`dataset'", text replace
	
	*open table
	file open tablecontents using ./output/16_hhClassif_forForestPlot_`outcome'_`dataset'.txt, t w replace
	
	*write table title and column headers
	file write tablecontents "Wave: `dataset', Outcome: `outcome'" _n
	file write tablecontents _tab _tab _tab _tab _tab ("MV adjusted incl HH size") _n
	file write tablecontents _tab _tab ("N") _tab ("Events") _tab ("% with event") _tab ("HR") _tab ("low CI") _tab ("upper CI") _n
