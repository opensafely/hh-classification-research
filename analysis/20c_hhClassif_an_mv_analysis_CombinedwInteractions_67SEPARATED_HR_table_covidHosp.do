...
foreach outcome in covidHosp {
	...
	* Open a log file
	capture log close
	log using "./logs/20c_hhClassif_an_mv_analysis_CombinedwInteractions_67SEPARATED_HR_table_`outcome'_`dataset'", text replace
	
	*open dataset
	use ./output/hhClassif_analysis_dataset_STSET_`outcome'_ageband_3`dataset'.dta, clear
	
	*open table
	file open tablecontents using ./output/20c_hhClassif_an_mv_analysis_CombinedwInteractions_67SEPARATED_HR_table_`outcome'_`dataset'.txt, t w replace
