...
foreach outcome in nonCovidDeath  {
	...
	* Open a log file
	capture log close
	log using "./logs/20d_hhClassif_an_mv_an_wInteractions_67alone_HR_`outcome'_`dataset'", text replace
	
	*open dataset
	use ./output/hhClassif_analysis_dataset_STSET_`outcome'_ageband_3`dataset'.dta, clear
	
	*open table
	file open tablecontents using ./output/20d_hhClassif_an_mv_an_wInteractions_67alone_HR_`outcome'_`dataset'.txt, t w replace
...
*CODE BEFORE MAKING KRISHNAN'S CHANGES
/*
local dataset `1' 
...
foreach outcome in covidHospOrDeath  {
	...
	*need to account for different models for wave 1 (only interaction is with hhrisk) versus wave 2 (multiple interactions)
	if "`dataset'"=="MAIN" {
		...
		*age-adjusted
		capture noisily stcox i.hhRiskCatExp_5cats##i.eth5 i.ageCatfor67Plus, strata(utla_group) vce(cluster hh_id)
